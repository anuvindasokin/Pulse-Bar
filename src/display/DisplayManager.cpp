...
void DisplayManager::metricIcon(bool views,CRGB color){if(views){pixel(0,3,color);pixel(1,2,color);pixel(1,4,color);pixel(2,2,color);pixel(2,3,color);pixel(2,4,color);pixel(3,2,color);pixel(3,4,color);pixel(4,3,color);}else{pixel(29,1,color);pixel(28,2,color);pixel(29,2,color);pixel(30,2,color);pixel(27,4,color);pixel(28,3,color);pixel(29,3,color);pixel(30,3,color);pixel(31,4,color);pixel(27,5,color);pixel(31,5,color);}}
void DisplayManager::showMetric(int64_t v,bool views,CRGB color){clear();String s=String(v);if(s.length()>5)s=s.substring(0,5);int areaStart=views?6:0,areaWidth=26,x=areaStart+max(0,(areaWidth-(int)s.length()*4+1)/2);for(char c:s){glyph(c,x,color);x+=4;}metricIcon(views,color);present();}
void DisplayManager::showMetricGradient(int64_t v,bool views,CRGB start,CRGB middle,CRGB end){clear();String s=String(v);if(s.length()>5)s=s.substring(0,5);int areaStart=views?6:0,areaWidth=26,x=areaStart+max(0,(areaWidth-(int)s.length()*4+1)/2),count=s.length();for(int i=0;i<count;i++){uint8_t pos=count<2?0:(uint8_t)(i*255/(count-1));CRGB color=pos<128?blend(start,middle,pos*2):blend(middle,end,(pos-128)*2);glyph(s[i],x,color);x+=4;}metricIcon(views,views?start:end);present();}

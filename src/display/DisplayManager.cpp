...
void DisplayManager::metricIcon(bool views,CRGB color){static const uint8_t eye[5]={0b0011100,0b0110110,0b1101011,0b0110110,0b0011100};static const uint8_t person[7]={0b0011100,0b0111110,0b0111110,0b0011100,0b0111110,0b1111111,0b1100011};const uint8_t* rows=views?eye:person;int height=views?5:7,x0=views?0:25,y0=views?1:0;for(int y=0;y<height;y++)for(int x=0;x<7;x++)if(rows[y]&(1<<(6-x)))pixel(x0+x,y0+y,color);}
void DisplayManager::showMetric(int64_t v,bool views,CRGB color){clear();String s=String(v);if(s.length()>5)s=s.substring(0,5);int areaStart=views?8:0,areaWidth=24,x=areaStart+max(0,(areaWidth-(int)s.length()*4+1)/2);for(char c:s){glyph(c,x,color);x+=4;}metricIcon(views,color);present();}
void DisplayManager::showMetricGradient(int64_t v,bool views,CRGB start,CRGB middle,CRGB end){clear();String s=String(v);if(s.length()>5)s=s.substring(0,5);int areaStart=views?8:0,areaWidth=24,x=areaStart+max(0,(areaWidth-(int)s.length()*4+1)/2),count=s.length();for(int i=0;i<count;i++){uint8_t pos=count<2?0:(uint8_t)(i*255/(count-1));CRGB color=pos<128?blend(start,middle,pos*2):blend(middle,end,(pos-128)*2);glyph(s[i],x,color);x+=4;}metricIcon(views,views?start:end);present();}

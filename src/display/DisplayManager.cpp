...
void DisplayManager::metricIcon(bool views,CRGB color,int x0){static const uint8_t eye[3]={0b01110,0b10101,0b01110};static const uint8_t person[4]={0b010,0b000,0b111,0b111};const uint8_t* rows=views?eye:person;int height=views?3:4,width=views?5:3,y0=views?2:2;for(int y=0;y<height;y++)for(int x=0;x<width;x++)if(rows[y]&(1<<(width-1-x)))pixel(x0+x,y0+y,color);}
void DisplayManager::showMetric(int64_t v,bool views,CRGB color){clear();String s=String(v);if(s.length()>5)s=s.substring(0,5);int numberWidth=s.length()*4-1,iconWidth=views?5:3,groupWidth=iconWidth+2+numberWidth,start=max(0,(32-groupWidth)/2),x=views?start+iconWidth+2:start;for(char c:s){glyph(c,x,color);x+=4;}metricIcon(views,color,views?start:start+numberWidth+2);present();}
void DisplayManager::showMetricGradient(int64_t v,bool views,CRGB startColor,CRGB middle,CRGB end){clear();String s=String(v);if(s.length()>5)s=s.substring(0,5);int numberWidth=s.length()*4-1,iconWidth=views?5:3,groupWidth=iconWidth+2+numberWidth,start=max(0,(32-groupWidth)/2),x=views?start+iconWidth+2:start,count=s.length();for(int i=0;i<count;i++){uint8_t pos=count<2?0:(uint8_t)(i*255/(count-1));CRGB color=pos<128?blend(startColor,middle,pos*2):blend(middle,end,(pos-128)*2);glyph(s[i],x,color);x+=4;}metricIcon(views,views?startColor:end,views?start:start+numberWidth+2);present();}

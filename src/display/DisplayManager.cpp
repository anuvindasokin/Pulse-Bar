...
void DisplayManager::showMetric(int64_t v,bool views,CRGB color){clear();String s=String(v);if(s.length()>5)s=s.substring(0,5);int numberWidth=s.length()*4-1,iconWidth=views?5:4,groupWidth=iconWidth+1+numberWidth,start=max(0,(32-groupWidth)/2),x=views?start+iconWidth+1:start;for(char c:s){glyph(c,x,color);x+=4;}metricIcon(views,color,views?start:start+numberWidth+1);present();}
void DisplayManager::showMetricGradient(int64_t v,bool views,CRGB startColor,CRGB middle,CRGB end){clear();String s=String(v);if(s.length()>5)s=s.substring(0,5);int numberWidth=s.length()*4-1,iconWidth=views?5:4,groupWidth=iconWidth+1+numberWidth,start=max(0,(32-groupWidth)/2),x=views?start+iconWidth+1:start,count=s.length();for(int i=0;i<count;i++){uint8_t pos=count<2?0:(uint8_t)(i*255/(count-1));CRGB color=pos<128?blend(startColor,middle,pos*2):blend(middle,end,(pos-128)*2);glyph(s[i],x,color);x+=4;}metricIcon(views,views?startColor:end,views?start:start+numberWidth+1);present();}
